...
| `temperatureCoefficientBands` | array of `{coefficient, temperatureRange}` | No | ppm/°C | **Class 1 only, per-span figures.** For dielectrics whose datasheet prints a different coefficient per temperature span (Murata U2J: -55..25 °C and 25..125 °C). Each item: `coefficient` (dimensionWithTolerance, raw ppm/°C) + `temperatureRange` `{minimum, maximum}` (°C), both required; at least two items. A single figure goes in `temperatureCoefficient`. Forbidden on `ceramic-class-2`. |
...
- The schema enforces the one conditional it can state cleanly: a `ceramic-class-2` part MUST NOT carry `temperatureCoefficient` or `temperatureCoefficientBands`. The converse is not enforced — a Class-1 or film part may legitimately quote a percent-over-range figure as well.
